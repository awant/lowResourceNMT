# 1) set ROOT_DATA_DIR (should point to folder from git repo: ROOT_DATA_DIR=/media/roman/Elements/LowResourceNMT)
# 2) download he-en data to DATA_DIR path (should exists: DATA_DIR/en.train.txt, DATA_DIR/en.test.txt for training)
# 3) set TRAIN_SIZE (in 1000, for example: 236 means 236k. 236 is about 80% of all dataset, set TRAIN_SIZE <= 236 or change test/dev sizes)
# 4) run this script
