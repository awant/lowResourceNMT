...
def get_checkpoint_path():
  checkpoints = []
  checkpoint_file = FLAGS.checkpoint_path + "checkpoint"
  with open(checkpoint_file, encoding='utf-8', newline='') as fckpt:
    for line in fckpt:
      ...
      if ckpt_type == 'all_model_checkpoint_paths':
        checkpoints.append(ckpt_name.strip(' \n\"'))
  return checkpoints
...
def decode(estimator, hparams, decode_hp):
  if FLAGS.decode_interactive:
    ...
    for checkpoint_elem in get_checkpoint_path():
      decoding.decode_from_file(estimator, FLAGS.decode_from_file, hparams,
                                decode_hp, FLAGS.decode_to_file + checkpoint_elem,
                                checkpoint_path=FLAGS.checkpoint_path + checkpoint_elem)
      if FLAGS.keep_timestamp:
        ckpt_time = os.path.getmtime(FLAGS.checkpoint_path + checkpoint_elem + ".index")
        os.utime(FLAGS.decode_to_file + checkpoint_elem, (ckpt_time, ckpt_time))
  '''
  else:
